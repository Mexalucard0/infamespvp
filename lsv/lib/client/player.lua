...
local function processPassiveMode()
	local playerPedId = PlayerPedId()

	if DoesEntityExist(playerPedId) then
		if Player.InPassiveMode then
			FreezeEntityPosition(playerPedId, not _allowMovement)
		else
			FreezeEntityPosition(playerPedId, false)
		end

		World.EnablePvp(not Player.InPassiveMode)

		SetEntityAlpha(playerPedId, Player.InPassiveMode and 192 or 255)
		SetPlayerInvincible(PlayerId(), Player.InPassiveMode)
	end
end
...
function Player.IsInAnyEvent()
	local player = Player.ServerId()
	return player == World.HotPropertyPlayer or player == World.BeastPlayer or player == World.KingOfTheCastlePlayer
end
...
function Player.HasAnyGarage()
	return next(Player.Garages) ~= nil
end
...
function Player.HasAnyDrugBusiness()
	return next(Player.DrugBusiness) ~= nil
end
...
function Player.GetCrewMembersCount()
	return table.length(Player.CrewMembers)
end
...
function Player.GetWeaponStats(weaponHash)
	return Player.WeaponStats[tostring(weaponHash)] or 0
end
...
function Player.TeleportAsync(position, isInInterior)
	Screen.FadeOutAsync(1000)
	...
	SetPedCoordsKeepVehicle(playerPed, position.x, position.y, position.z - 1.)
	if type(position) == 'table' and position.heading then
		SetEntityHeading(playerPed, position.heading)
	end

	local collisionTimer = Timer.New()
	while not HasCollisionLoadedAroundEntity(playerPed) and collisionTimer:elapsed() <= 5000 do
		Citizen.Wait(0)
	end
	PlaceObjectOnGroundProperly(playerPed)

	Player.IsInInterior = isInInterior

	Screen.FadeInAsync(750)
end
...
function Player.SetModelAsync(skinModel, isTemp)
	...
	if Player.Loaded then
		...
		SetEntityMaxHealth(playerPed, Settings.maxPlayerHealth)
...
function Player.GetVehicleName(vehicleIndex)
	...
	return vehicle.userName or VehicleUtility.GetVehicleData(vehicle.model).name or Vehicle.GetModelName(vehicle.model)
end

function Player.GetVehiclePrimaryColor(vehicleIndex)
	local vehicle = Player.Vehicles[vehicleIndex]
	return vehicle.colors and vehicle.colors.primary or 0
end

function Player.GetVehicleSecondaryColor(vehicleIndex)
	local vehicle = Player.Vehicles[vehicleIndex]
	return vehicle.colors and vehicle.colors.secondary or 0
end

function Player.GetVehicleWheelType(vehicleIndex)
	local vehicle = Player.Vehicles[vehicleIndex]
	return vehicle.wheels and vehicle.wheels.type or -1
end

function Player.GetVehicleMod(vehicleIndex, modType)
	local vehicle = Player.Vehicles[vehicleIndex]
	return vehicle.mods and vehicle.mods[tostring(modType)] or -1
end
...
RegisterNetEvent('lsv:initChallenges')
AddEventHandler('lsv:initChallenges', function(challenges)
	Player.Challenges = challenges
end)

RegisterNetEvent('lsv:challengeUpdated')
AddEventHandler('lsv:challengeUpdated', function(id, count)
	Player.Challenges[id] = count
end)
...
RegisterNetEvent('lsv:drugBusinessRemoved')
AddEventHandler('lsv:drugBusinessRemoved', function(type)
	Player.DrugBusiness[type] = nil
end)
