...
local function tryDeleteEntity(entity, toNetFunc)
	...
	if not _netIds[netId] or not _netIds[netId].delete or not NetworkDoesEntityExistWithNetworkId(netId) or not Network.RequestControlOfNetworkId(netId) then
		return
	end
...
function Network.RequestControlOfNetworkId(netId)
...
function Network.CreatePed(pedType, modelHash, position, heading, data)
	...
	local ped = CreatePed(pedType, modelHash, position.x, position.y, position.z, heading or math.random(0., 360.), true, true)
...
function Network.CreatePedInsideVehicle(vehicleNet, pedType, modelHash, seat, data)
	...
	if not NetworkDoesNetworkIdExist(vehicleNet) or not NetworkDoesEntityExistWithNetworkId(vehicleNet) or not Network.RequestControlOfNetworkId(vehicleNet) then
		return nil
	end
...
function Network.CreateVehicle(modelHash, position, heading, data)
	...
	local vehicle = CreateVehicle(modelHash, position.x, position.y, position.z, heading or math.random(0., 360.), true, true)
