...
local function getSurvivalRecord(survivalId)
	...
	if recordData then
		return recordData.PlayerName..' | '..recordData.Waves..' Oleadas'
	else
		return '-'
	end
end
...
AddEventHandler('lsv:survivalFinished', function(success, reason)
	...
	Gui.FinishMission('Supervivencia', success, reason)
end)
...
AddEventHandler('lsv:startSurvival', function(id)
	...
	Gui.StartMission('Supervivencia', 'Sobrevive y acaba con los enemigos.')
	...
	Citizen.CreateThread(function()
		while true do
			...
			if _location.waveIndex > _location.waveCount then
				...
				Gui.DrawBar('OLEADAS COMPLETADAS', _location.waveIndex - 1, barIndex)
				barIndex = barIndex + 1

				if Player.Records[_survivalId] then
					Gui.DrawBar('RECORD PERSONAL', Player.Records[_survivalId], barIndex)
					barIndex = barIndex + 1
				end

				if _survivalRecords[_survivalId] then
					Gui.DrawBar('MEJOR DEL SERVIDOR', _survivalRecords[_survivalId].Waves, barIndex)
					barIndex = barIndex + 1
				end
			end

			if nextWaveTimer then
				Gui.DisplayObjectiveText('Prepárate para la próxima oleada de enemigos.')
				Gui.DrawTimerBar('SIGUIENTE OLEADA EN', Settings.survival.waveInterval - nextWaveTimer:elapsed(), 1)
			else
				Gui.DisplayObjectiveText('Elimina a los ~r~enemigos~w~.')
				Gui.DrawProgressBar('OLEADA '.._location.waveIndex..' DE '..#_waveSettings, (_location.totalEnemyCount - _location.enemiesLeft) / _location.totalEnemyCount, 1)
			end
		end
	end)
	...
	while true do
		...
		if Player.DistanceTo(_location.position, true) > Settings.survival.radius then
			TriggerEvent('lsv:finishSurvival', false, 'Has abandonado el area de la mision.')
			return
		end
		...
		if not enemyWaveSpawned then
			...
			elseif nextWaveTimer:elapsed() > Settings.survival.waveInterval then
				...
				for i = 1, waveBoostLevel do
					if math.random(0, 100) < _endlessWaveBoost.bossChance then
						_location.bossCount = _location.bossCount + 1
					end
				end

				if _location.enemies then
					for _, pedNet in ipairs(_location.enemies) do
						Network.DeletePed(pedNet)
					end
				end
				...
				while #_location.enemies ~= _location.totalEnemyCount do
					...
					local location = table.irandom(_location.spawnPoints)
					local isBoss = bossCount < _location.bossCount
					local modelHash = isBoss and _bossSettings.model or table.irandom(_location.pedModels)
					...
					local weaponHash = table.irandom(isBoss and _bossSettings.weapons or waveData.weapons)
				...
				Gui.DisplayPersonalNotification('Oleada '..tostring(_location.waveIndex))
				nextWaveTimer = nil
...
AddEventHandler('lsv:init', function()
	...
	Gui.CreateMenu('survival', 'Supervivencia')
	WarMenu.SetTitleBackgroundColor('survival', table.unpack(Color.ORANGE))

	table.foreach(_locations, function(location)
		location.blip = Map.CreatePlaceBlip(Blip.SURVIVAL, location.position.x, location.position.y, location.position.z, 'Supervivencia', Color.BLIP_ORANGE)
	end)
	...
	while true do
			...
			if isPlayerInFreeroam then
				...
				if World.GetDistance(playerPosition, location.position, true) <= Settings.placeMarker.radius then
					if not WarMenu.IsAnyMenuOpened() then
						Gui.DisplayHelpText('Presiona ~INPUT_TALK~ para iniciar la mision de Supervivencia.')
		...
		if WarMenu.IsMenuOpened('survival') then
			...
			elseif WarMenu.Button('Mejor del servidor', getSurvivalRecord(_survivalMenuId)) then
			elseif WarMenu.Button('Record personal', getPlayerSurvivalBest(_survivalMenuId)) then
			end
...
AddEventHandler('lsv:init', function()
	World.AddPedHandler(function(ped)
		...
		elseif not DoesBlipExist(blip) then
			...
			Map.SetBlipText(blip, isBoss and 'Jefe' or 'Enemigo')
