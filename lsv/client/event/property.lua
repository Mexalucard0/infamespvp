...
local _instructionsText = 'Recoje el maletín y manténlo el mayor tiempo posible para obtener una recompensa..'

local _propertyData = nil

local function createBriefcase(position)
	_propertyData.pickup = CreatePickupRotate(`PICKUP_MONEY_CASE`, position.x, position.y, position.z, 0.0, 0.0, 0.0, 8, false, true)
...
AddEventHandler('lsv:startHotProperty', function(data, passedTime)
	...
	Citizen.CreateThread(function()
		if Player.IsInFreeroam() and not passedTime then
			Gui.StartEvent('Pago Caliente', _instructionsText)
		end
		...
		while true do
			...
			if Player.IsInFreeroam() then
				local eventObjectiveText = 'Recoge el ~g~maletín~w~ y cuidalo.'
				if World.HotPropertyPlayer then
					if World.HotPropertyPlayer == Player.ServerId() then
						eventObjectiveText = 'Manten al maletín el mayor tiempo posible.'
					else
						eventObjectiveText = Gui.GetPlayerName(World.HotPropertyPlayer, '~w~')..'<C> tiene el ~r~maletín~w~. Robaselo.</C>'
					end
				end
				...
				Gui.DrawTimerBar('FIN DEL EVENTO', math.max(0, Settings.property.duration - GetGameTimer() + _propertyData.startTime), 1)
...
AddEventHandler('lsv:finishHotProperty', function(winners)
	...
	local messageText = Gui.GetPlayerName(winners[1], '~p~')..' Gano Pago Caliente.'
	if playerPosition then
		messageText = 'Ganaste Pago Caliente con una puntuacion de '..playerTime
	end
