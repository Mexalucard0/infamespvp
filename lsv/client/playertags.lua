	...
	while true do
		...
		for _, id in ipairs(players) do
			...
			if id == PlayerId() then
				...
				local blipAlpha = NetworkIsInSpectatorMode() and 0 or 255
				if GetBlipAlpha(blip) ~= blipAlpha then
					SetBlipAlpha(blip, blipAlpha)
				end
			elseif NetworkIsPlayerActive(id) then
				...
				if isPlayerActive then
					...
					isPlayerHasBounty = playerData.killstreak >= Settings.pvp.bounty.killstreak
				...
				if not gamerTag or gamerTag.ped ~= ped or not IsMpGamerTagActive(gamerTag.tag) then
					...
					SetMpGamerHealthBarMax(tag, GetEntityHealth(ped), Settings.maxPlayerHealth)
				...
				if isPlayerCrewMember then
					color = 9
				elseif isPlayerHotProperty or isPlayerBeast or isPlayerKingOfTheCastle or isPlayerHasBounty then
					color = 6
				...
				SetMpGamerTagHealthBarColour(tag, color)

				local isGamerTagVisible = isHealthBarVisible
				if not isGamerTagVisible then
					isGamerTagVisible = isPlayerActive and not isPlayerDead and blipAlpha ~= 0 and HasEntityClearLosToEntity(playerPed, ped, 17)
				end

				SetMpGamerTagVisibility(tag, 0, isGamerTagVisible) -- GAMER_NAME
				SetMpGamerTagVisibility(tag, 2, isHealthBarVisible) -- HEALTH/ARMOR
				SetMpGamerTagVisibility(tag, 4, isGamerTagVisible and isPlayerTalking and Settings.voice.enabled) -- AUDIO_ICON
				...
				if ped ~= 0 then
					...
					local scale = 0.7
					if isPlayerHotProperty or isPlayerKingOfTheCastle or isPlayerBeast or isPlayerHasBounty then
						scale = 0.9
					...
					elseif isPlayerHotProperty or isPlayerKingOfTheCastle or isPlayerBeast or isPlayerHasBounty then
					...
					if not isPlayerActive then
						blip = 0
					elseif not isPlayerCrewMember then
						if not isPlayerHotProperty and not isPlayerBeast and not isPlayerKingOfTheCastle and not isPlayerHasBounty and GetWeapontypeGroup(playerWeapon) ~= -1212426201 then
